...
class Surface(Mobject):
    ...
    def init_points(self):
        ...
        for (du, dv) in [(0, 0), (self.epsilon, 0), (0, self.epsilon)]:
            uv_grid = np.array([[[u + du, v + dv] for v in v_range]
                                for u in u_range])
            point_grid = np.apply_along_axis(lambda p: self.uv_func(*p), 2,
                                             uv_grid)
    ...
    def pointwise_become_partial(self,
        ...
        self.set_points(
            np.vstack([
                self.get_partial_points_array(arr.copy(),
                                              a,
                                              b, (nu, nv, 3),
                                              axis=axis)
    ...
    def get_partial_points_array(
        ...
        if axis == 0:
            points[:lower_index] = interpolate(points[lower_index],
                                               ...
                                               lower_residue)
            points[upper_index + 1:] = interpolate(points[upper_index],
            ...
            points[:, :lower_index] = interpolate(points[:, lower_index],
                                                  points[:, lower_index + 1],
                                                  lower_residue).reshape(shape)
            points[:, upper_index + 1:] = interpolate(
                points[:, upper_index], points[:, upper_index + 1],
                upper_residue).reshape(shape)
...
class TexturedSurface(Surface):
    ...
    def init_points(self):
        ...
        self.data["im_coords"] = np.array([
            [u, v] for u in np.linspace(0, 1, nu)
    ...
    def pointwise_become_partial(self,
        ...
        im_coords[:] = self.get_partial_points_array(im_coords, a, b,
                                                     (nu, nv, 2), axis)
        return self
